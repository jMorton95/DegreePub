...
from helpers.print_colours import print_red, prompt_underline
...
class TicketInputValidationService:
    ...
    def prompt_and_validate_priority(self) -> int:
        ...
        while True:
            ...
            try:
                ...
                print_red(f"{user_input} is not a number. ")
                continue

            if not (priority >= MIN_PRIORITY and priority <= MAX_PRIORITY):
                print_red(f"{user_input} is not between {MIN_PRIORITY} and {MAX_PRIORITY}. ")
                continue
    ...
    def prompt_and_validate_ticket_type(self) -> TicketType:
        ...
        while True:
            user_input = prompt_underline(f"Please enter a ticket type. Valid Options: {', '.join([i.name for i in TicketType])} ")

            try: 
                ticket_type = TicketType[user_input.upper()]
            except KeyError:
                print_red(f"{user_input} is not a valid Ticket Type. ")
                continue
    ...
    def prompt_for_string_of_minimum_length(self, prompt: str, length: int) -> str:
        ...
        while True:
            user_input = prompt_underline(f"Please enter a {prompt} longer than {length} character{'s' if length > 1 else ''} ")

            if not (len(user_input) >= length):
                print_red(f"'{user_input}' is not longer than {length} character{'s' if length > 1 else ''} ")
                continue
            else:
                return user_input
    ...
    def prompt_for_initial_estimate(self) -> float:
        ...
        while True:
            user_input = prompt_underline(f"Please enter an initial estimate of hours/minutes as a Float (e.g. '5.5' = 5h30m). ")
